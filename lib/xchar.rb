# cribbed unabashedly from Sam Ruby.
# see: http://intertwingly.net/blog/2005/09/28/XML-Cleansing
...
class Fixnum
  ...
  def xchr
    ...
    # now return "\x??" instead of "*" for invalids
    if XChar::VALID.find {|range| range.include? n}
      XChar::PREDEFINED[n] or (n<128 ? n.chr : "&##{n};")
    elsif n < 16
      "\\x0#{n.to_s(16)}"
    else
...
class TestXmlEscaping < Test::Unit::TestCase
  ...
  def test_invalid
    ...
    assert_equal '\x00',   "\x00".to_xs               # null
    assert_equal '\x0c',   "\x0C".to_xs               # form feed
    assert_equal '\xffff', "\xEF\xBF\xBF".to_xs       # U+FFFF
  end
